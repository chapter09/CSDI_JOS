...
trap_init(void)
{
	...
	extern void entry_dir_err();     
	extern void entry_div_err();
	extern void entry_deb_exc();
	extern void entry_nmi();
	extern void entry_brkpt();
	extern void entry_ovflow();
	extern void entry_bound();
	extern void entry_illop();
	extern void entry_dev();
	extern void entry_dfault();
	extern void entry_tss();
	extern void entry_segnp();
	extern void entry_stack();
	extern void entry_gpfault();
	extern void entry_pgfault();
	extern void entry_fp_err();
...
//	ts->ts_esp0 = KSTACKTOP - num * (KSTKSIZE +KSTKGAP);
	ts->ts_esp0 = (uint32_t)percpu_kstacks[num];
	ts->ts_ss0 = GD_KD;
	...
	gdt[(GD_TSS0 >> 3) + num] = SEG16(STS_T32A, (uint32_t) (ts),
					sizeof(struct Taskstate), 0);
	gdt[(GD_TSS0 >> 3) + num].sd_s = 0;
...
//	cprintf("##get the trap to dispatch\n");
	...
	if(tf->tf_trapno == T_PGFLT){
		...
		if(tf->tf_regs.reg_eax == SYS_exofork) {
			tf->tf_regs.reg_eax 
				= syscall(tf->tf_regs.reg_eax, 0, 0, 0, 0, 0);
			return;
		}
	}
	...
	if(curenv->env_pgfault_upcall != NULL){
		...
		*((uint32_t *)esp) = fault_va;
		...
		curenv->env_tf.tf_eip 
			= (uintptr_t)(curenv->env_pgfault_upcall);
